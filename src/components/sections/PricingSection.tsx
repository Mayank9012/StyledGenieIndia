    ...
    return (
        ...
        <Container marginLeft="5vw" marginRight="5vw">
            ...
            {plans.map((plan, index) => (
                <div key={index} className={`${(activePlanIndex == index)? 'bg-gray-300': 'bg-white'}  rounded-lg overflow-hidden border-2 border-gray-900`} onClick={() => setActivePlanIndex(index)}>
                <div className="p-8">
                    <h4 className={`${typography.heading.h5} mb-5 font-bold`}>{plan.title}</h4>
                    <div className="mb-7">

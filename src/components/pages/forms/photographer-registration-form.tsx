    ...
    switch (name) {
      ...
      case 'portfolioFile':
        if (!value) {
          ...
          newErrors.portfolioFile = 'File size exceeds 10MB limit';
          isValid = false;
        }
        else if (!['application/pdf', 'application/msword', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document', 'application/zip'].includes(value.type)) {
          newErrors.portfolioFile = 'Invalid file format. Accepted formats: PDF, DOC, DOCX, ZIP';
          isValid = false;
        }
        else {
      ...
      case 'workingHours':
        ...
        if (!value.trim()) {
          ...
          isValid = false;
        } else if (isNaN(hours) || hours < 1 || hours > 168) {
          newErrors.workingHours = 'Please enter a valid number between 1-168';
          isValid = false;
        } else {
...
const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
  ...
  return (
    <div className="min-h-screen bg-g-to-b from-be-50 to-iigo-100 py-12 px-4 sm:px-6 lg:px-8 ">
      <div className="max-w-3xl mx-auto bg-white rounded-xl shadow-lg overflow-hidden">
        <div className="bg-gray-600 py-6 px-8">
          <h1 className="text-2xl font-bold text-white">Freelance Photographer Registration</h1>
          <p className="text-indigo-100 mt-2">Join our network of photographers</p>
        </div>

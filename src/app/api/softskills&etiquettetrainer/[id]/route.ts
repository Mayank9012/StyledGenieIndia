...
export async function GET(
  ...
  try {
    ...
    if (!photographer) {
      return NextResponse.json({ error: 'softskillsetiquetteTrainers not found' }, { status: 404 });
    }
    ...
    console.error('Error fetching softskillsetiquetteTrainers:', error);
    return NextResponse.json({ 
      error: 'Failed to retrieve softskillsetiquetteTrainers data' 
    }, { status: 500 });
  }
}